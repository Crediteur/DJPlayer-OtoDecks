/*
  ==============================================================================
	...
	Created: 26 Aug 2022 11:05:22am
	Author:  Zty
...
void PlaylistComponent::paint(Graphics& g)
{
	...
	g.setFont(14.0f);
	g.drawText("", getLocalBounds(), Justification::centred, true);   // draw some placeholder text
}
...
void PlaylistComponent::paintCell(Graphics& g, int rowNumber, int columnId, int width, int height, bool rowIsSelected)
{
		...
		if (columnId == 3)
		{
			...
			if (!searchBox.isEmpty())
			{
				if (rowNumber < getNumRows()) // prevent vector out of range
...
{
	// create input stream from saved session
	std::ifstream savedPlaylist("saved-playlist.csv");
	std::string filePath;
	std::string length;

	// read data, create new objs by line
